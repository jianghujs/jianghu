...
module.exports = option => {
  return async (ctx, next) => {
    ...
    const allUserGroupRolePageList = await jianghuKnex(tableEnum._user_group_role_page).select();
    const isNotPublic = !allUserGroupRolePageList.find(rule => rule.group === 'public'
        && rule.role === '*' && rule.page === pageId);
    const { originalUrl } = ctx.request;
    const originalUrlEncode = encodeURIComponent(originalUrl);

    const goToErrorPage = ({ isLoginUser, error }) => {
      if (!isLoginUser) {
        ctx.redirect(ctx.app.config.loginPage + `?redirectUrl=${originalUrlEncode}`);
      } else {
        const { errorCode, errorReason } = error;
        ctx.redirect(`${ctx.app.config.helpPage}?errorCode=${errorCode}&errorReason=${errorReason}`);
      }
    };
    ...
    if (appType === 'multiApp') {
      ...
      if (isNotPublic && !targetUserApp) {
        goToErrorPage({ isLoginUser, error: errorInfoEnum.request_app_forbidden });
        return;
      }
    }
